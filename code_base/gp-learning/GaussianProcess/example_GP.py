...
def main(opts):
    ...
    real_pos = X_test[:, 0]
    real_vel = X_test[:, 1]
    #* GP predictions should be compared with labels (y), not inputs (X) because the inputs skip certain processings like normalization and scalings
    ...
    # this is dead reckoning, it's not going to be a useful as an evaluation tool if it's done for long horizons because it is open-loop and model errors will grow.
    for i, delta in enumerate(pos_pred_mean_delta):
        if i > 0:
            pos_pred_mean[i] = pos_pred_mean[i-1] + delta
    # especially not for acceleration predictions on the hydraulics system using the same (pos, vel, u) inputs. The sampling rate is too slow and the measurements are too noisy from double differentiation. For velocity/position control, you don't even need to do this.
