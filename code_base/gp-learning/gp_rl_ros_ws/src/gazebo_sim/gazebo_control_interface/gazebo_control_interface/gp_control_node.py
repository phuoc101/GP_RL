...
class SteeringActionClient(Node):
    ...
    def __init__(self, opts):
        ...
        self.joint_state_sub = self.create_subscription(
            DynamicJointState, "dynamic_joint_states", self.dynamic_state_callback, 10
        )
    ...
    def init_model(self):
        ...
        config = {
            **config,
            "GP_training_iter": self.opts.gp_train_iter,
...
def main(args=None):
    ...
    parser.add_argument(
        "--force-train",
        action="store_false",
        help="Force retraining GP model",
    )
