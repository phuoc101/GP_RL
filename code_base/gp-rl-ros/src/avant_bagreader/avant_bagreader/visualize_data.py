...
class DataVisualizer(Node):
    ...
    def visualize_data(self):
        self.logger.debug(f"Position readings count: {len(self.data['X1_xvu'][:,0])}")
        self.logger.debug(f"Velocity readings count: {len(self.data['X1_xvu'][:,1])}")
        self.logger.debug(f"Input cmd count: {len(self.data['X1_xvu'][:,2])}")
        title = "Plots with self.data from dataset {}, frequency {}".format(
        ...
        ax[0, 1].plot(self.data["timestamp"], self.data["Y1"], label="d_boom_pos (rad)")
        ax[1, 0].plot(self.data["timestamp"], self.data["Y2"], label="d_boom_vel")
        ax[1, 1].plot(np.diff(self.data["timestamp"]), label="timestmp_diff (sec)")
        for a in ax:
            for x in a:
                x.legend()
        plt.legend()
